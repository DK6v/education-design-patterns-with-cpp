...
class A : public ObjectBase, public printable<A> {
public:
    ...
    virtual std::unique_ptr<ObjectBase> clone() const { return std::unique_ptr<ObjectBase>(new A(*this)); }
};
...
TEST(ObjectFactory, Test05) {
    A a;
    A b;
    b = a;
    std::cout << "-- whoami: " << b.whoami() << std::endl;

}

TEST(ObjectFactory, Test06) {
    B a;
    B b;
    b = a;
    std::cout << "-- whoami: " << b.whoami() << std::endl;
