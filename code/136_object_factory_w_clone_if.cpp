...
public:
    ...
    std::unique_ptr<ObjectBase> clone() const;
};

/* It is not possible for ObjectBase to inherit from ICloneable,
 * otherwise it will be implemented twice by the ObjectBase and Cloneable
 */
class IClonable {
public:
...
std::unique_ptr<ObjectBase> ObjectBase::clone() const {
    /* 1. Type of dynamyc_cast cannot be incomplete type.
     * 2. Derived objects MUST implement Clonable in order to be cloned,
     *    otherwise dynamic_cast will return nullptr.
     */
    return dynamic_cast<const IClonable*>(this)->clone();
}

template <typename T>
class Clonable : public IClonable {
public:
...
public:
    ...
    using Clonable<A>::clone;
};

class B : public ObjectBase,
          public Clonable<B>,
          public printable<B> {
public:
...
private:
    ObjectFactory() = default; // singleton
...
TEST(ObjectFactory, Test01) {
    auto pa = ObjectFactory::getInstance().makeObject(A_Builder());
    std::cout << "-- whoami: " << pa->whoami() << std::endl;

    auto pb = ObjectFactory::getInstance().makeObject(B_Builder());
    std::cout << "-- whoami: " << pb->whoami() << std::endl;

    auto pc = ObjectFactory::getInstance().makeObject(C_Builder());
    std::cout << "-- whoami: " << pc->whoami() << std::endl;
}

TEST(ObjectFactory, Test02) {
    auto pa = ObjectFactory::getInstance().makeObject(A_Builder());
    auto cpa = pa->clone();
    std::cout << "-- whoami: " << cpa->whoami() << std::endl;

    auto pb = ObjectFactory::getInstance().makeObject(B_Builder());
    auto cpb = pb->clone();
    std::cout << "-- whoami: " << cpb->whoami() << std::endl;

    auto pc = ObjectFactory::getInstance().makeObject(C_Builder());
    auto cpc = pc->clone();
    std::cout << "-- whoami: " << cpc->whoami() << std::endl;
