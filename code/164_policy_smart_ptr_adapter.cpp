...
// Use adapter to hide the delete policy.

template <typename T,
          template <typename> typename ReleasePolicy = NoRelease>
class smart_array_ptr : public smart_ptr<T, DeleteArray, ReleasePolicy> {
    // Only the default constructor is available by default,
    // include constructor from the base class;
    using smart_ptr<T, DeleteArray, ReleasePolicy>::smart_ptr;
};
...
template <typename T,
          template <typename> typename ReleasePolicy = NoRelease>
using smart_array_ptr_t = smart_ptr<T, DeleteArray, ReleasePolicy>;
...
TEST(StrategyTemplate, Test07) {
    {
        smart_array_ptr<Base> ptr(new Base[2]);
    }
    std::cout << "--" << std::endl;
}

TEST(StrategyTemplate, Test08) {
    {
        smart_array_ptr_t<Base> ptr(new Base[2]);
    }
    std::cout << "--" << std::endl;
}
