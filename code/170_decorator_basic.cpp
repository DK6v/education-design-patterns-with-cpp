...
class Decorator_A : public Base {
public:
    Decorator_A(Base& base) : Base(), base_(base) {}
...
class Decorator_B : public Base {
public:
    Decorator_B(Base& base) : Base(), base_(base) {}
...
TEST(Decorator, Test01) {
    {
        ...
        std::shared_ptr<Base> da(new Decorator_A(*base));
        da->whoami();

        std::shared_ptr<Base> db(new Decorator_B(*da));
        db->whoami();
    }
